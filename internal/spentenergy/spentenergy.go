...
func WalkingSpentCalories(steps int, weight, height float64, duration time.Duration) float64 {
	...
	mnSpeed := MeanSpeed(steps, duration)

	ccals := ((walkingCaloriesWeightMultiplier * weight) + (mnSpeed*mnSpeed/height)*walkingSpeedHeightMultiplier) * duration.Hours() * minInH
...
func RunningSpentCalories(steps int, weight float64, duration time.Duration) float64 {
	...
	ccals := ((runningCaloriesMeanSpeedMultiplier * mnSpeed) - runningCaloriesMeanSpeedShift) * weight
	return ccals
}
